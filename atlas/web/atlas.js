...
renderer.domElement.addEventListener("pointerdown", (ev) => {
  pointerDownAt = { x: ev.clientX, y: ev.clientY, t: Date.now() };
});
renderer.domElement.addEventListener("pointerup", (ev) => {
  ...
  pointerDownAt = null;
  if (Math.hypot(dx, dy) > 4 || dt > 300) return; // it was a drag
...
function applyGraph(graph) {
  ...
  for (const n of graph.nodes) {
    ...
    if (!item) {
      ...
      mesh.add(halo);
      item = { data: n, mesh, pos: mesh.position,
        vel: new THREE.Vector3() };
      state.nodes.set(n.id, item);
    } else {
      item.data = n;
      const c = colorFor(n.kind);
      item.mesh.material.color.setHex(c);
      item.mesh.material.emissive.setHex(c);
    }
  }
...
function physicsStep() {
  ...
  if (nodes.length > 600) return; // skip for very large graphs
  const REPEL = 280;
  const SPRING = 0.0015;
  const CENTER = 0.001;
...
async function selectNode(id) {
  ...
  const node = await brain.getNode(id);
  if (!node) { info.textContent = "Node not found."; return; }
  const neigh = await brain.neighbors(id);
  info.innerHTML = `
    ...
    <h3>connected to (${neigh.length})</h3>
    <ul>${neigh.map((n) =>
      `<li><a href="#" data-id="${n.id}">${escapeHtml(n.label)}</a>
       <span class="kind">(${escapeHtml(n.relation || "")},
       w=${(n.weight || 0).toFixed(2)})</span></li>`).join("")
       || "<li><em>nothing yet</em></li>"}</ul>
  `;
}
...
document.getElementById("chatform").addEventListener("submit", async (ev) => {
  ...
  input.value = "";
  appendMsg("user", msg);
  await brain.rememberChat("user", msg);
  const data = await brain.answer(msg);
  appendMsg("atlas", data.reply, data.refs || []);
  await brain.rememberChat("atlas", data.reply, data.refs || []);
});

(async () => {
  for (const row of await brain.recentChats(50)) {
    appendMsg(row.role === "user" ? "user" : "atlas", row.content,
      row.node_refs || []);
  }
})();
...
async function renderThoughts() {
  const list = await brain.recentThoughts(30);
  const ul = document.getElementById("thoughtlist");
  ul.innerHTML = "";
  for (const t of list.slice().reverse()) {
    const li = document.createElement("li");
    const when = new Date(t.created_at).toLocaleTimeString();
...
async function renderAgents() {
  ...
  for (const a of list) {
    ...
    div.innerHTML = `
      ...
      <div class="hint">every ${a.schedule_seconds}s ·
        <span class="status ${a.status}">${a.status}</span>
        ${a.last_run ? "· last "
          + new Date(a.last_run).toLocaleTimeString() : ""}
      </div>
      <pre>${escapeHtml(a.last_output || "(no output yet)")}</pre>
      <div class="row">
        <button data-act="run" data-id="${a.id}">Run now</button>
        <button data-act="del" data-id="${a.id}" class="danger">
          Delete</button>
      </div>
    `;
...
document.getElementById("agentlist").addEventListener("click", async (ev) => {
  ...
  if (b.dataset.act === "run") await agents.runNow(id);
  else if (b.dataset.act === "del") {
    if (!confirm("Delete this agent?")) return;
    await brain.deleteAgent(id);
  }
  renderAgents();
});
...
document.getElementById("agent-create").addEventListener("click", async () => {
  ...
  await brain.saveAgent({ name, purpose, code, schedule_seconds });
...
(async function boot() {
  const seeded = await brain.seedIfEmpty();
  await agents.installStarterAgents();
